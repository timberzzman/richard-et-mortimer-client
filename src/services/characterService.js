...
export async function getCharactersByID(ids) {
  const data = await request(import.meta.env.VITE_RM_API, charactersByIdQuery, { ids });
  return data.charactersByIds;
}

export async function getCharacter(id) {
  const data = await request(import.meta.env.VITE_RM_API, characterQuery, { id });
  return data.character;
}

export async function getCompleteCharacter(id) {
  const data = await request(import.meta.env.VITE_RM_API, completeCharacterQuery, { id });
  return data.character;
}
